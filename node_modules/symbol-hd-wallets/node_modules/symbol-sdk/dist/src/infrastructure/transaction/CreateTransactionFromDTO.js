...
const extractMessage = (message) => {
    ...
    if (message) {
        ...
        const messageType = message.type !== undefined ? message.type : format_1.Convert.hexToUint8(message.substring(0, 2))[0];
        if (messageType === MessageType_1.MessageType.PlainMessage) {
            msgObj = PlainMessage_1.PlainMessage.createFromPayload(messagePayload);
        }
        else if (messageType === MessageType_1.MessageType.EncryptedMessage) {
            msgObj = EncryptedMessage_1.EncryptedMessage.createFromPayload(messagePayload);
        }
        else if (messageType === MessageType_1.MessageType.PersistentHarvestingDelegationMessage) {
            msgObj = PersistentHarvestingDelegationMessage_1.PersistentHarvestingDelegationMessage.createFromPayload(messagePayload);
        }
    }
    return msgObj;
};
