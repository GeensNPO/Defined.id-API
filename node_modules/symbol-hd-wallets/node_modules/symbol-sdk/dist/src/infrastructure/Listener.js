...
class Listener {
        ...
        switch (channelName) {
            ...
            case ListenerChannelName.partialAdded:
                this.messageSubject.next({
                    ...
                    message: CreateTransactionFromDTO_1.CreateTransactionFromDTO(message.data),
                });
                break;
    ...
    finalizedBlock() {
        ...
        return this.messageSubject.asObservable().pipe(operators_1.share(), operators_1.filter((_) => _.channelName === ListenerChannelName.finalizedBlock), operators_1.filter((_) => _.message instanceof FinalizedBlock_1.FinalizedBlock), operators_1.map((_) => _.message));
    }
    /**
    ...
    transactionSubscription(channel, address, transactionHash) {
        ...
        return this.messageSubject.asObservable().pipe(operators_1.filter((listenerMessage) => listenerMessage.channelName === channel), operators_1.filter((listenerMessage) => listenerMessage.message instanceof Transaction_1.Transaction), operators_1.switchMap((_) => {
            const transactionObservable = rxjs_1.of(_.message).pipe(operators_1.filter((transaction) => this.filterHash(transaction, transactionHash)));
    ...
    transactionHashSubscription(channel, address, transactionHash) {
        this.subscribeTo(`${channel}/${address.plain()}`);
        return this.messageSubject.asObservable().pipe(operators_1.filter((_) => _.channelName === channel), operators_1.filter((_) => typeof _.message === 'string'), operators_1.filter((_) => _.channelParam.toUpperCase() === address.plain()), operators_1.map((_) => _.message), operators_1.filter((_) => !transactionHash || _.toUpperCase() == transactionHash.toUpperCase()));
    }
    /**
    ...
    status(address, transactionHash) {
        ...
        return this.messageSubject.asObservable().pipe(operators_1.filter((_) => _.channelName === ListenerChannelName.status), operators_1.filter((_) => _.message instanceof TransactionStatusError_1.TransactionStatusError), operators_1.filter((_) => _.channelParam.toUpperCase() === address.plain()), operators_1.map((_) => _.message), operators_1.filter((_) => !transactionHash || _.hash.toUpperCase() == transactionHash.toUpperCase()));
    }
    /**
    ...
    cosignatureAdded(address) {
        ...
        return this.messageSubject.asObservable().pipe(operators_1.filter((_) => _.channelName.toUpperCase() === ListenerChannelName.cosignature.toUpperCase()), operators_1.filter((_) => _.message instanceof CosignatureSignedTransaction_1.CosignatureSignedTransaction), operators_1.filter((_) => _.channelParam.toUpperCase() === address.plain()), operators_1.map((_) => _.message));
    }
    /**
     * @internal
