...
class TransferTransaction extends Transaction_1.Transaction {
     ...
     */
    message = PlainMessage_1.EmptyMessage, signature, signer, transactionInfo) {
    ...
    static createFromPayload(payload, isEmbedded = false) {
        ...
        }), TransferTransaction.createMessageFromBuffer(builder.getMessage()), networkType, isEmbedded ? new UInt64_1.UInt64([0, 0]) : new UInt64_1.UInt64(builder.fee.amount), isEmbedded || signature.match(`^[0]+$`) ? undefined : signature, signerPublicKey.match(`^[0]+$`) ? undefined : PublicAccount_1.PublicAccount.createFromPublicKey(signerPublicKey, networkType));
    ...
    validate() {
        var _a;
        if (((_a = this.message) === null || _a === void 0 ? void 0 : _a.type) === MessageType_1.MessageType.PersistentHarvestingDelegationMessage) {
    ...
    getMessageBuffer() {
        ...
        return this.message.type === MessageType_1.MessageType.PersistentHarvestingDelegationMessage || !this.message.payload
            ? payloadBuffer
     ...
     * @internal
     */
    static createMessageFromBuffer(messageBuffer) {
        if (!messageBuffer.length) {
            return PlainMessage_1.EmptyMessage;
        }
        const messageType = messageBuffer[0];
        const messageHex = format_1.Convert.uint8ToHex(messageBuffer).substring(2);
        switch (messageType) {
            case MessageType_1.MessageType.PlainMessage:
                return PlainMessage_1.PlainMessage.createFromPayload(messageHex);
            case MessageType_1.MessageType.EncryptedMessage:
                return EncryptedMessage_1.EncryptedMessage.createFromPayload(messageHex);
            case MessageType_1.MessageType.PersistentHarvestingDelegationMessage:
                return PersistentHarvestingDelegationMessage_1.PersistentHarvestingDelegationMessage.createFromPayload(messageHex);
            default:
