...
const FinalizationEpochDto_1 = require("catbuffer-typescript/dist/FinalizationEpochDto");
class VotingKeyLinkTransaction extends Transaction_1.Transaction {
     ...
     */
    static create(deadline, linkedPublicKey, startEpoch, endEpoch, linkAction, networkType, maxFee = new UInt64_1.UInt64([0, 0]), signature, signer) {
        return new VotingKeyLinkTransaction(networkType, TransactionVersion_1.TransactionVersion.VOTING_KEY_LINK, deadline, maxFee, linkedPublicKey, startEpoch, endEpoch, linkAction, signature, signer);
    }
    /**
    ...
    static createFromPayload(payload, isEmbedded = false) {
        ...
        const transaction = VotingKeyLinkTransaction.create(isEmbedded ? Deadline_1.Deadline.create() : Deadline_1.Deadline.createFromDTO(builder.getDeadline().timestamp), format_1.Convert.uint8ToHex(builder.getLinkedPublicKey().votingKey), builder.getStartEpoch().finalizationEpoch, builder.getEndEpoch().finalizationEpoch, builder.getLinkAction().valueOf(), networkType, isEmbedded ? new UInt64_1.UInt64([0, 0]) : new UInt64_1.UInt64(builder.fee.amount), isEmbedded || signature.match(`^[0]+$`) ? undefined : signature, signerPublicKey.match(`^[0]+$`) ? undefined : PublicAccount_1.PublicAccount.createFromPublicKey(signerPublicKey, networkType));
