...
export class GlobalKeyValueSetBuilder {
    /** Key value array. */
     ...
     */
    public constructor(keys: GlobalKeyValueBuilder[]) {
        this.keys = keys;
    }
    ...
    public static loadFromBinary(payload: Uint8Array): GlobalKeyValueSetBuilder {
        ...
        const keys: GlobalKeyValueBuilder[] = [];
        for (let i = 0; i < (Array.isArray(keyValueCount) ? GeneratorUtils.compact(keyValueCount) : keyValueCount); i++) {
            const item = GlobalKeyValueBuilder.loadFromBinary(Uint8Array.from(byteArray));
            keys.push(item);
     ...
     */
    public getKeys(): GlobalKeyValueBuilder[] {
        return this.keys;
    }
